...
export def TabsOrBuffers(_opts: dict<any>): string
  ...
  if add_left_tabs
    var [tab, tabwidth] = g:CrystallineTabFn(tabselidx, tabbufs[tabselidx - 1], max_tab_width, v:false)
    if enable_sep
  ...
  if add_right_tabs
    var [tab, tabwidth] = g:CrystallineTabFn(tabselidx + 2, tabbufs[tabselidx + 1], max_tab_width, v:false)
    if enable_mouse
  ...
  while tabidx >= 0 && width < max_width && tab_count < max_tabs
    var [tab, tabwidth] = g:CrystallineTabFn(tabidx + 1, tabbufs[tabidx], max_tab_width, v:false)
    if enable_sep
      tab ..= tab_sep
      tabwidth += sep_width
    endif
